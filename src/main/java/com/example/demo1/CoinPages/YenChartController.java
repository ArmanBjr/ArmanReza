...
public class YenChartController {
    ...
    private void updateChart() {
        if (situ == 2) {
            ...
            for (int i = 23; i >= 0; i--) {
                ...
                String query = "SELECT AVG(yen) as avg_yen FROM currency_rates WHERE " +
                ...
                try (Connection conn = DataBase.connectDb();
                    ...
                    if (rs.next()) {
                        double avgPrice = rs.getDouble("avg_yen");
    ...
    private void updateChartForLast30Days() {
        ...
        for (int i = 29; i >= 0; i--) {
            ...
            String query = "SELECT AVG(yen) as avg_yen FROM currency_rates WHERE date = '" + dateString + "'";
    ...
    public void updateLabels() {
        try (Connection connection = DataBase.connectDb()) {
            ...
            if (resultSet.next()) {
                ...
                Price.setText(String.valueOf(yen));
                LowestPrice.setText(String.valueOf(Values.getLowestPrice(connection, "yen", formattedDate, formattedTime)));
                HighestPrice.setText(String.valueOf(Values.getHighestPrice(connection, "yen", formattedDate, formattedTime)));
                ChangePercentage.setText(String.valueOf(Values.getChangePercentage(connection, "yen", formattedDate, formattedTime)));
                if (Values.getChangePercentage(connection, "yen", formattedDate, formattedTime) >= 0) {
